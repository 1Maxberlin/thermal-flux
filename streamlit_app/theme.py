...
# ----------------------------------------------------------------------
hero(
    title="FlowThermal Suite — a working Engineering toolkit",
    subtitle="PE 262 Capstone — pipe hydraulics, heat transfer and core data, in one place",
    tagline="Three engineering workflows, one shared physics engine, zero guesswork. "
            "Size a line, predict a pressure drop, check how fast a vessel cools, and "
            "interrogate core-analysis data — all with transparent, verifiable physics "
            "and exportable results.",
    pills=["Live", "3 modules", "Hand-calc verified"],
    accent_word="Engineering",
)
...
# ----------------------------------------------------------------------
# Module cards (A, B, C) — light cards with checklists
# ----------------------------------------------------------------------
    ...
    st.markdown(
        module_card(
            ...
            desc="Velocity, Reynolds number, Colebrook friction factor and "
                 "Darcy–Weisbach pressure drop, with a ΔP vs Q sweep plot.",
            tag="Fluid mechanics",
            accent="cyan-blue",
            delay=0.05,
            checklist=["Velocity, Re, f and ΔP", "ΔP vs Q sweep chart", "One-click CSV export"],
        ),
...
with col2:
    st.markdown(
        module_card(
            ...
            accent="blue-violet",
            delay=0.15,
            checklist=["Wall heat loss and flux", "Time to reach a target", "Real-time cooling curve"],
        ),
        unsafe_allow_html=True,
    )
...
with col3:
    st.markdown(
        module_card(
            ...
            accent="violet-flame",
            delay=0.25,
            checklist=["CSV upload and preview", "Histogram and crossplot", "Filtered CSV download"],
        ),
        unsafe_allow_html=True,
    )
...
the UI.
    ...
    st.markdown(
        module_card(
            ...
            accent="amber-flame",
            delay=0.35,
            checklist=["Step-by-step workflows", "Equations in full", "Units and assumptions"],
        ),
        unsafe_allow_html=True,
    )
